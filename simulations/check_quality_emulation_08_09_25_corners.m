...
lupstatus = array2table(readmatrix("D:\Pesquisa\Pesquisa_2024\Sustainable_Prawn\Emulation_2025\inventory_post_run_08_09_25.txt"));
...
if isempty(find(cornerall.ras_sub==ras_sub & cornerall.vid==vid)) == 0
    ...
    %store outputs
    output_analysis = [output_analysis ; [ras_sub vid corner height(rolling_out_sim_tab) prawn_meat_price viability_rate RAS_units_average RAS_meat_production RAS_sub_total cal_AL protein_AL carbon_avoided_total carbon_released_total wood_created delta_out alpha_out irate_out 12*(1+cs_chitin) 50*(1+cs_PLJ) 2.4*(1+cs_feed) 0.984*(1+cs_salt)]];
