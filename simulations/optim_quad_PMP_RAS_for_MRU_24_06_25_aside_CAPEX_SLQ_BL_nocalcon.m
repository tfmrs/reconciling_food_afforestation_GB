...
if treeless  ==0
    obj_fun_sim_val = vec_REV_crop(id_act_crop)'*sol_sim.area_crop(id_act_crop) - 1/2*calib_crop(:,1)'*sol_sim.area_crop(id_act_crop).^2 + mat_GM_nofor_forage_m(i,id_act_forage)*sol_sim.area_forage(id_act_forage) - 1/2*calib_forage(:,1)'*sol_sim.area_forage(id_act_forage).^2 + vec_REV_tree(id_act_tree)'*sol_sim.area_tree(id_act_tree) + (sub_set_aside+topup_set_aside)'*(ones(1,n_act_tree)*sol_sim.area_tree(id_act_tree) - table2array(area_observed_tree(i,id_act_tree))*ones(n_act_tree,1)) - 1/2*calib_tree(:,1)'*sol_sim.area_tree(id_act_tree).^2
...
%        GM_gain_RAS_net = -(fval_target - fval_sim) + area_RAS *vec_GM_RAS_ex_lab - (vec_size_RAS*vec_VARCOS_RAS_oplab*(floor((g-1)/5)+1) + vec_size_RAS*vec_VARCOS_RAS_mglab*(floor((g-1)/10)+1) + RAS_CAPEX_annuity)
        ...
        kcal_sim = vec_kcal_crop(id_act_crop)'*sol_sim.area_crop(id_act_crop) + mat_kcal_m(i,id_act_forage)*sol_sim.area_forage(id_act_forage) + vec_kcal_RAS*area_RAS
...
if treeless == 0
    rolling_out_sim_RAS_by_unit = [rolling_out_sim_RAS_by_unit ; [i g sol_sim.area_crop' sol_sim.area_forage' sol_sim.area_tree' sollives' GM_gain_RAS_net (fval_target - fval_sim) (area_RAS*vec_GM_RAS) fval_sim obj_fun_sim_val share_land_RAS share_labour_RAS share_water_RAS area_RAS kcal_sim kcal_BL share_land_set_aside share_labour_set_aside share_water_set_aside share_kcal_RAS sub_aside]]
else
    rolling_out_sim_RAS_by_unit = [rolling_out_sim_RAS_by_unit ; [i g sol_sim.area_crop' sol_sim.area_forage' repelem(NaN,2) sollives' GM_gain_RAS_net (fval_target - fval_sim) (area_RAS*vec_GM_RAS) fval_sim obj_fun_sim_val share_land_RAS share_labour_RAS share_water_RAS area_RAS kcal_sim kcal_BL share_land_set_aside share_labour_set_aside share_water_set_aside share_kcal_RAS sub_aside]]
end    
        %    else
