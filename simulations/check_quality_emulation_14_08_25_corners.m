 ...
 %to deal with mistaken 2nd stages
   try
       ...
       dna = join(dna_own,dna,"Keys","Var3");
        ...
        rollfini = lupstatus.finished(find(lupstatus.ras_sub==ras_sub & lupstatus.vid==vid))
        rollciwi = lupstatus.c_ws_pre(find(lupstatus.ras_sub==ras_sub & lupstatus.vid==vid))
        
        if (rollfini == 0 & rollciwi == 1)
    ...
    wood_created = sum(rolling_out_sim_tab.Broadleaved + rolling_out_sim_tab.Conifer,"omitnan") - sum(area_observed_tree.broadleaved(rolling_tab.i) + area_observed_tree.conifer(rolling_tab.i),"omitnan")
      ...
      %Run food carbon      
           ...
           analysis_food_carbon_module_14_08_25 
