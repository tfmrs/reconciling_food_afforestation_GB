...
%%%UPLOAD PRICE DATA
externalfile_coeffs = readtable("coefficients_OF_constraints_(EQ)_EQ_price_07_05_25.txt")
...
%load("input_data_calibration_woodquad_16_06_2025")
...
%                    simulation_module_water_labour_MLbased_new_sims_23_06_25
%                    simulation_module_water_labour_MLbased_new_sims_22_07_25
                     simulation_module_water_labour_EMULATION_new_sims_24_07_25
                end
    end 
...
%%%%%%END OF LOOP
save(strcat(output_directory,"ws_pre_",extractBefore(type_sim,4),"_ras_sub_",string(ras_sub),"_",string(price_meat),"__",timing,".mat"))

taboutR = array2table([price_meat sum(rolling_out_sim_RAS(:,2))*RAS_yield_meat])
taboutR.Properties.VariableNames = ["price_meat" "agg_quantity_meat"]
%%Demand quantity and distance
x_intercept =  71228242.25182
y_intercept =   23.5610
d0=x_intercept
d1=-x_intercept/y_intercept			

quantity_demand = d0 + d1*price_meat
distance = (taboutR.agg_quantity_meat - quantity_demand)^2

taboutR.distance = distance
taboutR.distance_prc = abs((taboutR.agg_quantity_meat - quantity_demand)/quantity_demand)

writetable(taboutR,strcat(output_directory,"ws_pre_",extractBefore(type_sim,4),"_ras_sub_",string(ras_sub),"_",string(price_meat),"__",timing,".txt"))
