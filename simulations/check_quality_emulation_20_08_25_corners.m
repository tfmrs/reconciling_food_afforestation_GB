...
%%keep non finished as want to open aside-o
lupstatus2 = lupstatus2(find(lupstatus2.rep==1 | lupstatus2.end_or_corner == 1),:)
size(lupstatus2)

%removing repeat of finished
lupstatus2.check = repelem(0,height(lupstatus2))'

for iij = 2:height(lupstatus2)
    if (lupstatus2.key(iij-1) == lupstatus2.key(iij))
...
if isempty(find(cornerall.ras_sub==ras_sub & cornerall.vid==vid)) == 0
    AVC_price_EQ = cornerall.price_EQ(find(cornerall.ras_sub==ras_sub & cornerall.vid==vid))
else
    AVC_price_EQ = NaN
    "the run is not a all-above-demand-case"
...
isnan(NaN)       
    ...
    wood_created = sum(rolling_out_sim_tab.Broadleaved + rolling_out_sim_tab.Conifer,"omitnan") - sum(area_observed_tree.broadleaved(rolling_tab.i) + area_observed_tree.conifer(rolling_tab.i),"omitnan");
      ...
      %Run food carbon      
           ...
           analysis_food_carbon_module_18_08_25;

    %store outputs
    output_analysis = [output_analysis ; [ras_sub vid corner height(rolling_out_sim_tab) prawn_meat_price viability_rate RAS_units_average RAS_meat_production RAS_sub_total wood_created cal_AL protein_AL carbon_total index_3_carbon_effect index_4_externality_effect delta_out alpha_out irate_out 12*(1+cs_chitin) 50*(1+cs_PLJ) 2.4*(1+cs_feed) 0.984*(1+cs_salt)]];
