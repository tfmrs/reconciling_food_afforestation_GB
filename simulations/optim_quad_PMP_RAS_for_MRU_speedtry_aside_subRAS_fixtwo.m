...
if  RAS_CAPEX*(g-d_turn_RAS_on*(ras_sub-1)-d_turn_RAS_on_two_units*(ras_sub-1))*(1-alpha)/(NPV_RAS*delta) <= NM_BL_LQ*d_turn_RAS_on
    ...
    %%%LQ  
%    NM_AL_LQ = vec_GM_crop(id_act_crop)'*area_crop(id_act_crop) + mat_GM_forage_m(i,id_act_forage)*area_forage(id_act_forage) + (sub_set_aside+topup_set_aside)*area_set_aside + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i)
