...
RAS_CAPEX_annuity = RAS_CAPEX*g/NPV_RAS;
...
if treeless == 0
    NM_BL_LQ = (vec_GM_crop(id_act_crop)')*sol_target.area_crop(id_act_crop) + (mat_GM_forage_m(i,id_act_forage))*sol_target.area_forage(id_act_forage) + sol_target.area_tree(id_act_tree)*vec_GM_tree' - activity_shares.total_fixed_cost_ex_lab_pix(i);
else
    NM_BL_LQ = (vec_GM_crop(id_act_crop)')*sol_target.area_crop(id_act_crop) + (mat_GM_forage_m(i,id_act_forage))*sol_target.area_forage(id_act_forage) - activity_shares.total_fixed_cost_ex_lab_pix(i);
end

if  RAS_CAPEX*g*(1-alpha)/(NPV_RAS*delta) <= NM_BL_LQ*d_turn_RAS_on
    ...
    if treeless == 0
       obj_fun_sim = vec_REV_crop(id_act_crop)'*area_crop(id_act_crop) - 1/2*calib_crop(:,1)'*area_crop(id_act_crop).^2 + mat_GM_nofor_forage_m(i,id_act_forage)*area_forage(id_act_forage) - 1/2*calib_forage(:,1)'*area_forage(id_act_forage).^2 + vec_REV_tree(id_act_tree)'*area_tree(id_act_tree) - 1/2*calib_tree(:,1)'*area_tree(id_act_tree).^2;
     else
    ...
    %%%%constraints, with trees
        ...
        area_const = ones(n_act_crop,1)'*area_crop(id_act_crop) + ones(n_act_forage,1)'*area_forage(id_act_forage) + ones(n_act_tree,1)'*area_tree(id_act_tree) + area_RAS <= table2array(area_observed_crop(i,id_act_crop))*ones(n_act_crop,1) + table2array(area_observed_forage(i,id_act_forage))*ones(n_act_forage,1) + table2array(area_observed_tree(i,id_act_tree))*ones(n_act_tree,1);
    
        %%Labour
        %labour_const = sum(vec_SLR(1:10)*area) <= sum(vec_SLR(1:10).*table2array(area_observed(i,:)))
        labour_const = vec_SLR_crop(id_act_crop)'*area_crop(id_act_crop) + mat_SLR_forage_m(i,id_act_forage)*area_forage(id_act_forage) + mat_SLR_tree(id_act_tree)'*area_tree(id_act_tree) + vec_SLR_RAS*g*vec_size_RAS <= table2array(area_observed_crop(i,id_act_crop))*vec_SLR_crop(id_act_crop) + table2array(area_observed_forage(i,id_act_forage))*mat_SLR_forage_m(i,id_act_forage)' + table2array(area_observed_tree(i,id_act_tree))*mat_SLR_tree(id_act_tree);
    
        %%%Water
        water_const = vec_water_crop(id_act_crop)'*area_crop(id_act_crop) + mat_water_forage_m(i,id_act_forage)*area_forage(id_act_forage) + mat_water_tree(id_act_tree)'*area_tree(id_act_tree) + vec_water_RAS*area_RAS <= table2array(area_observed_crop(i,id_act_crop))*vec_water_crop(id_act_crop) + table2array(area_observed_forage(i,id_act_forage))*mat_water_forage_m(i,id_act_forage)' + table2array(area_observed_tree(i,id_act_tree))*mat_water_tree(id_act_tree);
    else
    %%%%constraints, without trees
        %%Area
        %area_const_arable = ones(10,1)'*area <= table2array(area_observed(i,:))*ones(10,1)
        area_const = ones(n_act_crop,1)'*area_crop(id_act_crop) + ones(n_act_forage,1)'*area_forage(id_act_forage) + area_RAS <= table2array(area_observed_crop(i,id_act_crop))*ones(n_act_crop,1) + table2array(area_observed_forage(i,id_act_forage))*ones(n_act_forage,1);
    
        %%Labour
        %labour_const = sum(vec_SLR(1:10)*area) <= sum(vec_SLR(1:10).*table2array(area_observed(i,:)))
        labour_const = vec_SLR_crop(id_act_crop)'*area_crop(id_act_crop) + mat_SLR_forage_m(i,id_act_forage)*area_forage(id_act_forage) + vec_SLR_RAS*g*vec_size_RAS <= table2array(area_observed_crop(i,id_act_crop))*vec_SLR_crop(id_act_crop) + table2array(area_observed_forage(i,id_act_forage))*mat_SLR_forage_m(i,id_act_forage)';
    
        %%%Water
        water_const = vec_water_crop(id_act_crop)'*area_crop(id_act_crop) + mat_water_forage_m(i,id_act_forage)*area_forage(id_act_forage) + vec_water_RAS*area_RAS <= table2array(area_observed_crop(i,id_act_crop))*vec_water_crop(id_act_crop) + table2array(area_observed_forage(i,id_act_forage))*mat_water_forage_m(i,id_act_forage)';
    end    
...
%    NM_BL_LQ = table2array(area_observed_crop(i,id_act_crop))*vec_GM_crop(id_act_crop) + table2array(area_observed_forage(i,id_act_forage))*mat_GM_forage_m(i,id_act_forage)' - activity_shares.total_fixed_cost_ex_lab_pix(i)
    ...
    %2.AL,GM
%   NM_AL_LQ = vec_GM_crop(id_act_crop)'*area_crop(id_act_crop) + mat_GM_forage_m(i,id_act_forage)*area_forage(id_act_forage) + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i)
if treeless == 0
    NM_AL_LQ = vec_GM_crop(id_act_crop)'*area_crop(id_act_crop) + mat_GM_forage_m(i,id_act_forage)*area_forage(id_act_forage) + vec_GM_tree(id_act_tree)'*area_tree(id_act_tree) + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i);
else
    NM_AL_LQ = vec_GM_crop(id_act_crop)'*area_crop(id_act_crop) + mat_GM_forage_m(i,id_act_forage)*area_forage(id_act_forage) + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i);
end
    ...
    %try 1 
%   NM_AL_LQ = obj_fun_sim + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i)
%try 2 
% if treeless == 0  
%        NM_AL_LQ = vec_REV_crop(id_act_crop)'*area_crop(id_act_crop) - 1/2*calib_crop(:,1)'*area_crop(id_act_crop).^2 + mat_GM_nofor_forage_m(i,id_act_forage)*area_forage(id_act_forage) - 1/2*calib_forage(:,1)'*area_forage(id_act_forage).^2 + vec_REV_tree(id_act_tree)'*area_tree(id_act_tree) - 1/2*calib_tree(:,1)'*area_tree(id_act_tree).^2 + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i)
% else
%        NM_AL_LQ = vec_REV_crop(id_act_crop)'*area_crop(id_act_crop) - 1/2*calib_crop(:,1)'*area_crop(id_act_crop).^2 + mat_GM_nofor_forage_m(i,id_act_forage)*area_forage(id_act_forage) - 1/2*calib_forage(:,1)'*area_forage(id_act_forage).^2 + area_RAS*vec_GM_RAS_ex_lab - (labcost_RAS_step + RAS_CAPEX_annuity) - activity_shares.total_fixed_cost_ex_lab_pix(i)
% end
...
%    LQ_const = (RAS_CAPEX*g - GM_AL_LQ)/NPV_RAS <= GM_BL_LQ
    ...
    LQ_const = RAS_CAPEX*g*((1-alpha)/NPV_RAS + alpha) <= (NM_AL_LQ + NM_BL_LQ)*d_turn_RAS_on;
...
if treeless  ==0
    obj_fun_sim_val = vec_REV_crop(id_act_crop)'*sol_sim.area_crop(id_act_crop) - 1/2*calib_crop(:,1)'*sol_sim.area_crop(id_act_crop).^2 + mat_GM_nofor_forage_m(i,id_act_forage)*sol_sim.area_forage(id_act_forage) - 1/2*calib_forage(:,1)'*sol_sim.area_forage(id_act_forage).^2 + vec_REV_tree(id_act_tree)'*sol_sim.area_tree(id_act_tree) - 1/2*calib_tree(:,1)'*sol_sim.area_tree(id_act_tree).^2;
...
%        rolling_out_sim_RAS_by_unit = [rolling_out_sim_RAS_by_unit ; [i g sol_sim.area_crop' sol_sim.area_forage' sollives' GM_gain_RAS_net (fval_target - fval_sim) (area_RAS*vec_GM_RAS) fval_sim obj_fun_sim_val share_land share_labour share_water area_RAS]]
if treeless == 0
    rolling_out_sim_RAS_by_unit = [rolling_out_sim_RAS_by_unit ; [i g sol_sim.area_crop' sol_sim.area_forage' sol_sim.area_tree' sollives' GM_gain_RAS_net (fval_target - fval_sim) (area_RAS*vec_GM_RAS) fval_sim obj_fun_sim_val share_land share_labour share_water area_RAS]];
else
    rolling_out_sim_RAS_by_unit = [rolling_out_sim_RAS_by_unit ; [i g sol_sim.area_crop' sol_sim.area_forage' repelem(NaN,2) sollives' GM_gain_RAS_net (fval_target - fval_sim) (area_RAS*vec_GM_RAS) fval_sim obj_fun_sim_val share_land share_labour share_water area_RAS]];
end    
        %    else
%        rolling_out_sim_RAS_by_unit = [rolling_out_sim_RAS_by_unit ; [i g repelem(NaN,27)]]
