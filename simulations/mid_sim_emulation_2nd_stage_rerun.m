...
upload_data_DGP_ACER_emulation_speed_try
...
%for i = randsample(1:height(area_observed_crop),20)
...
%                                writematrix([i string(start_time) string(end_time) string(total_time) length(gvec)],strcat(output_directory,"speed_col_",string(i),".txt"));
                        ...
                        if pixel_class == "crop_&_forage"
                                rolling_out_sim_RAS = [rolling_out_sim_RAS ; [i 0 sol_target.area_crop' sol_target.area_forage' repelem(NaN,2) sollives' 0 0 0 fval_target fval_target 0 0 0 0 kcal_BL kcal_BL 0 0 0 0 0 0]];
                            else    
                                rolling_out_sim_RAS = [rolling_out_sim_RAS ; [i 0 repelem(NaN,12) sol_target.area_forage' repelem(NaN,2) sollives' 0 0 0 fval_target fval_target 0 0 0 0 kcal_BL kcal_BL 0 0 0 0 0 0]];
                            end    
%                                end_time = datetime
%                                end_time.Format = 'yyyy/MM/dd HH:mm:ss.SSS'
