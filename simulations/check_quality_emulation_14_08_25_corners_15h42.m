...
if listofsims3.finished == 1
        load(string(table2array(listofsims3.name(jj))))
  ...
  %try-catch ends
    ...
    wood_created = sum(rolling_out_sim_tab.Broadleaved + rolling_out_sim_tab.Conifer,"omitnan") - sum(area_observed_tree.broadleaved(rolling_tab.i) + area_observed_tree.conifer(rolling_tab.i),"omitnan")
      ...
      %Run food carbon      
           ...
           analysis_food_carbon_module_14_08_25_16h48

    %store outputs
    output_analysis = [output_analysis ; [ras_sub vid height(rolling_out_sim_tab) prawn_meat_price viability_rate RAS_units_average RAS_meat_production RAS_sub_total wood_created cal_AL protein_AL carbon_agri_total carbon_prawn_total RAS_sub_total index_3_carbon_effect index_4_externality_effect delta_out alpha_out irate_out 12*(1+cs_chitin) 50*(1+cs_PLJ) 2.4*(1+cs_feed) 0.984*(1+cs_salt)]];
