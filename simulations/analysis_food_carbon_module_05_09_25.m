...
%%%14/01/25, calculation of RAS total expenditure
       if (type_sim == "set_aside_only_gis_nocalconstraint_EWCO")
           social_cost_RAS = 0
	  else
%%%OPEX ex-labour
...
%%%labour cost
    mat_wage_PAYE.i = (1:height(mat_wage_PAYE))'
    rolling_out_sim_tab = join(rolling_out_sim_tab,mat_wage_PAYE(:,["i","ratio_final"]),"Key","i")

    RAS_units_opt2 = table2array(rolling_out_sim_tab(:,["best_RAS_units"]))
    ratio_final_opt2 = table2array(rolling_out_sim_tab(:,["ratio_final"]))
    
	labcost_RAS_total = sum(ratio_final_opt2*12*1.*(floor((RAS_units_opt2-1)/5)+1) + 	ratio_final_opt2*12*(1.4).*(floor((RAS_units_opt2-1)/10)+1))
	labcost_RAS_vec = [rolling_out_sim_tab.best_RAS_units rolling_out_sim_tab.ratio_final ratio_final_opt2*12*1.*(floor((RAS_units_opt2-1)/5)+1) + 	ratio_final_opt2*12*(1.4).*(floor((RAS_units_opt2-1)/10)+1)]
...
%this the social cost of RAS
%OBS: (single) sign inversion for cost_adj_extra_sens and the old
%OPEX_without_labour, because this is the cost, not the GM
social_cost_RAS = (104168.22 - cost_adj_extra_sensitivity)*RAS_units + labcost_RAS_total + RAS_units*RAS_CAPEX/NPV_RAS
       end
...
%if RAS_sub_total > 0
%%%Output-emulation 7
index_1_calorie_effect = cal_AL/social_cost_RAS
%now RAS sub total
%%%Output-emulation 8
index_2_protein_effect = protein_AL /social_cost_RAS
%else
...
%%%Output-emulation 9
index_3_carbon_effect = (-1)*(carbon_agri_total_neg + carbon_prawn_total_neg)/social_cost_RAS
